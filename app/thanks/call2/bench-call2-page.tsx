...
const fallbackPhoneNumber =
  ...
  "+18882882203";
const fallbackRingbaCampaignId =
  ...
  if (icon === "flowers") {
    return (
      ...
      >
        ...
        <path
          d="M10.8 6h2.4v3.2h2.7v2.3h-2.7v5h-2.4v-5H8.1V9.2h2.7V6Z"
          fill="#eaf2ff"
        />
      </svg>
    );
...
function getVisiblePrintedPhoneNumber() {
  ...
  const phoneNodes = Array.from(
    document.querySelectorAll<HTMLElement>("[data-call2-ringba-phone]"),
  );
...
export default function BenchCall2Page({
  ...
  const resolvedRingbaCampaignId =
    ringbaCampaignId || fallbackRingbaCampaignId;
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Ringba can still replace the visible number.
    }
  }, [leadId]);
  ...
  return (
    <main className="min-h-screen bg-white text-[#101820]">
      ...
      <header className="border-b border-[#eef2f7] bg-white">
        <div className="mx-auto flex h-[64px] max-w-[980px] items-center justify-between gap-3 px-4">
          <Image
            ...
            height={54}
            priority
            src="/media/logo-cuentas-de-retiro.png"
            width={180}
          />
          <a
          ...
          >
            ...
            >
              ...
              <path
                className="call2-call-wave"
                d="M20.5 12c1.8.5 3.2 1.9 3.7 3.7"
              />
              <path
                className="call2-call-wave call2-call-wave-2"
                d="M22 7.2c3.4.9 6.1 3.6 7 7"
              />
              <path
                className="call2-call-wave call2-call-wave-3"
      ...
      <section className="mx-auto flex min-h-[calc(100vh-64px)] w-full max-w-[560px] flex-col px-5 py-7 text-center">
        ...
        <p className="mt-5 text-[14px] font-black uppercase tracking-[0.08em] text-[#2d6cdf]">
          Beneficios preaprobados
        </p>
        <h1 className="mt-2 text-[26px] font-black leading-[1.12] md:text-[34px]">
          {safeFirstName
            ? `${safeFirstName}, tu aplicación está lista`
          ...
          <div className="mt-4 grid gap-2.5 text-left" aria-live="polite">
            {calculatedBenefits
              .slice(
                0,
                Math.min(
                  resolvedBenefitCount + 1,
                  calculatedBenefits.length,
                ),
              )
              .map((benefit, index) => {
        ...
        <p className="mx-auto mt-5 max-w-[420px] text-[12px] leading-[1.45] text-[#7b8494]">
          Un agente licenciado revisará contigo los detalles de {goalLabel}. La
          llamada es gratuita y sin compromiso.
        </p>
      </section>
    </main>
  );
