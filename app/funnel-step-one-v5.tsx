...
const immigrationStatuses = [
  ...
  },
  {
    ...
    detail: "(Green Card)",
    value: "Residente permanente (Green Card)",
    amount: 27974,
  },
  {
    id: "other-status",
...
const submissionStorageKey = "bf_iul_v5_submission_id";
...
export default function FunnelStepOneV5({
  initialLocation,
  phoneHref,
  ...
  const [immigration_status, setImmigrationStatus] = useState<string | null>(
    null,
  );
  ...
  const visibleStepCount = hasDetectedUsLocation ? 5 : 6;
  const visibleCurrentStep =
    hasDetectedUsLocation && currentStep > 4 ? currentStep - 1 : currentStep;
  ...
  function selectAge(ageRange: (typeof ageRanges)[number]) {
    ...
    if (ageRange.id === "65-plus") {
      router.push("/iul-v5/rechazo");
      return;
    }

    const addedAmount = ageRange.amount;
  ...
  function selectImmigrationStatus(
    status: (typeof immigrationStatuses)[number],
  ) {
    if (immigration_status) return;

    if (status.id === "other-status") {
      router.replace("/iul-v5/estatus-no-disponible");
      return;
    }

    const locationAmount = hasDetectedUsLocation ? zipCodeAmount : 0;
    const addedAmount = status.amount + locationAmount;
    const nextAmount = currentAmount + addedAmount;
    const stepCount = Math.max(1, Math.round(addedAmount / 1000));

    setImmigrationStatus(status.value);
    setOdometerValues(buildSlotValues(currentAmount, nextAmount, stepCount));
    setCurrentAmount(nextAmount);
    setOdometerRun((run) => run + 1);
    sessionStorage.setItem("immigration_status", status.value);
    sessionStorage.setItem(
      "immigration_status_value",
    ...
    window.dispatchEvent(
      new CustomEvent("funnel:step-complete", {
        detail: {
          step: 3,
          funnel_id: funnelId,
          immigration_status: status.value,
          immigration_status_value: status.amount,
          projectedAmount: nextAmount,
        },
      }),
    );
  ...
  async function submitContact(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      nextParams.set("immigration_status", immigration_status);
    ...
    <main className="funnel-page">
      <PopUp1
        ...
        ringbaTags={{
          ...
          iul_v5_age_group: age_group || "",
          iul_v5_insurance_goal: insurance_goal || "",
          iul_v5_immigration_status: immigration_status || "",
            ...
            <div className="question-stage">
              ...
              >
                {currentStep === 1 ? (
                  ...
                  <>
                    <h2 id="step-3-title">
                      Selecciona tu situación actual en los Estados Unidos
                    </h2>
                    <div className="answer-list age-answer-list immigration-answer-list">
                      {immigrationStatuses.map((status) => (
                        <button
                          ...
                          key={status.id}
                          onClick={() => selectImmigrationStatus(status)}
                          type="button"
                        >
                          <span className="immigration-option-copy">
                            <span>{status.label}</span>
                            {"detail" in status ? (
                              <span className="immigration-option-detail">
                                {status.detail}
                              </span>
                            ) : null}
                          </span>
                  ...
                  <>
                    <h2 id="step-4-title">¿Cuál es tu ZIP code?</h2>
                  ...
                  <>
                    <h2 id="step-5-title">¿Cuál es tu nombre?</h2>
                  ...
                  <>
                    <h2 id="step-6-title">¿Cómo podemos contactarte?</h2>
                    <form
