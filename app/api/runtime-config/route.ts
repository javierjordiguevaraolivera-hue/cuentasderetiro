...
function mapConfig(values: Record<string, string>) {
  const payPerCallStatus = normalizeString(
    values.NEXT_PUBLIC_PAY_PER_CALL_STATUS,
  ).toUpperCase();
  const payPerCallStartTime = normalizeString(
    values.NEXT_PUBLIC_PAY_PER_CALL_START_TIME,
  );
  const payPerCallEndTime = normalizeString(
    values.NEXT_PUBLIC_PAY_PER_CALL_END_TIME,
  );
  const ringbaCampaignId = normalizeString(
    values.NEXT_PUBLIC_RINGBA_CAMPAIGN_ID,
  );
  const payPerCallPhoneNumber = normalizeString(
    values.NEXT_PUBLIC_PAY_PER_CALL_PHONE_NUMBER,
  );
  const hasCompletePayPerCallConfig =
    ...
    /^1?[2-9]\d{2}[2-9]\d{6}$/.test(
      payPerCallPhoneNumber.replace(/\D/g, ""),
    );

  if (!hasCompletePayPerCallConfig) return defaults;
